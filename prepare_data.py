...
DATA_PATH = "/home/huixin/DLProject_SRCNN/SRCNN-keras/Train/" #"/home/huixin/Desktop/training_images/"
TEST_PATH = "/home/huixin/DLProject_SRCNN/SRCNN-keras/Test/Set5/"
Random_Crop = 30
Patch_size = 32
label_size = 20
conv_side = 6
...
def prepare_data(_path):
    ...
    for i in range(nums):
        ...
        lr_img = cv2.resize(hr_img, (shape[1] // scale, shape[0] // scale)) #DELETE INTEGER DIVISION
...
def prepare_crop_data(_path):
    ...
    for i in range(nums):
        ...
        hr_img = hr_img[:, :, 0]
        shape = hr_img.shape
        print('hr_img: ',hr_img) #DELETE THIS LINE
        print('shape: ', shape) #DELETE THIS LINE
        print('scaling',shape[1] / scale) #DELETE THIS LINE

        # two resize operation to produce training data and labels
        lr_img = cv2.resize(hr_img, (shape[1] // scale, shape[0] // scale)) #REMOVE INTEGER DIVISION
        lr_img = cv2.resize(lr_img, (shape[1], shape[0]))

        width_num = (shape[0] - (BLOCK_SIZE - BLOCK_STEP) * 2) // BLOCK_STEP #REMOVE INTEGER DIVISION
        print('width num: ',width_num) #DELETE THIS LINE
        height_num = (shape[1] - (BLOCK_SIZE - BLOCK_STEP) * 2) // BLOCK_STEP #REMOVE INTEGER DIVISION
        print('height num:', height_num) #DELETE THIS LINE
